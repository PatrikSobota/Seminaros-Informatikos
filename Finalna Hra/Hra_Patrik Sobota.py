...
def pacmanmove(event):
    ...
    target_dy = random.randint(1, 9)
    target_dx = random.randint(1, 17)
    ...
    if mapa[target_y][target_x] != '#':
        ...
        platno.coords(duch, target_dx * cell_size + cell_size / 2, target_dy * cell_size + cell_size / 2)

    if target_x == target_dx and target_y == target_dy:
        okno.destroy()
        print('Prehral si, duch ťa chytil :(')
        print ('Počet pozbieraných mincí bol: '+ str(skore))


    for idx,m in enumerate (gulicky):
        if target_x == m[0] and target_y == m[1]:
            platno.delete(m[2])
            del gulicky[idx]
            skore = skore+1
            #print(gulicky)
...
gulicka = tkinter.PhotoImage (file= 'coin.gif')
